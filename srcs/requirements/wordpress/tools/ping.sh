#!/bin/bash
IFS=$'\n'
for line in $(cat /etc/php/8.2/fpm/pool.d/www.conf); do
  modified_line="${line/listen = \/run\/php\/php8.2-fpm.sock/listen = 9000}"
  echo "$modified_line" >> temp.txt
done

mv temp.txt /etc/php/8.2/fpm/pool.d/www.conf
